...
export interface DoubaoWorkHandoffOptions {
  endpointUrl: string;
  outputPath: string;
  authHeader?: string;
}

export interface DoubaoWorkHandoffReceipt {
  schema_version: 'mybrain-cn-doubao-work-handoff-v1';
  output_path: string;
  endpoint_url: string;
...
export function createDoubaoWorkHandoff(options: DoubaoWorkHandoffOptions): DoubaoWorkHandoffReceipt {
  const outputPath = requireAbsolute(options.outputPath, 'Doubao Work handoff output path');
  const endpoint = new URL(options.endpointUrl);
  if (endpoint.protocol !== 'https:') throw new Error('Doubao Work remote MCP endpoint must use HTTPS.');
  if (isPrivateHostname(endpoint.hostname)) throw new Error('Doubao Work cloud handoff refuses localhost or private-network endpoints.');
  ...
  writeJson(outputPath, {
    schema_version: 'mybrain-cn-doubao-work-registration-v1',
    product: '豆包工作伙伴',
    registration: {
      name: '@MyBrain',
      ...
      authentication: {
        ...
        value: '<set-inside-doubao-work-secret-field>',
      },
      visibility: 'self-only',
